...
### v1.2 (2026-06-29) — 파일 개별 제거 + 저장 위치 지정

- **파일 개별 제거**: 목록 각 행에 `×` 버튼. 이벤트 위임(`fileListEl` 단일 리스너,
  `closest('.remove')` + `data-id`)으로 처리하며, 제거 시 썸네일 URL을 해제하고
  변환 버튼 상태·다운로드 영역을 갱신한다. 변환 중 제거에도 안전하도록
  `handleConvert`는 `items` 스냅샷(`[...items]`)을 순회한다.
- **저장 위치 지정 (File System Access API)**: Chrome/Edge에서 1장은
  `showSaveFilePicker`로 파일명·위치를 직접 지정, 여러 장은 `showDirectoryPicker`로
  폴더를 골라 파일들을 그 폴더에 직접 저장한다(ZIP 불필요). 미지원 브라우저
  (Firefox/Safari)는 기존 다운로드/ZIP로 폴백하며, JSZip은 폴백용으로 유지한다.
  위치 선택을 취소(`AbortError`)하면 저장하지 않고 목록을 유지하고, 성공 시에만
  목록을 자동 초기화한다. 다운로드 버튼 라벨은 지원 여부에 따라 바뀐다.
